...
DB_PATH = os.path.join(
    os.path.dirname(os.path.dirname(__file__)), "data", "databases", "devpartner.db"
)
...
rows = conn.execute(
    "SELECT task_id, task_type, status, started_at FROM task_queue WHERE status='running'"
).fetchall()
cleaned = 0
for r in rows:
    ...
    if started_at:
        ...
        if age_s > 600:  # 10 minutes
            ...
            cleaned += 1
            sys.stdout.write(
                f"[CLEAN] {r['task_id'][:20]}... {r['task_type']} age={age_s / 3600:.1f}h\n"
            )

...
class CleanupService:
    ...
    def __init__(self):
        ...
        self._last_vacuum_time: Optional[datetime] = None
    ...
    def _physical_delete_expired(self) -> dict:
        ...
        if deleted > 0:
            logger.info(f"🧹 物理删除: {deleted} 条过期任务 (>{self._soft_delete_retention_days}天)")
    ...
    def _maybe_vacuum(self):
        ...
        try:
            ...
            logger.info(f"🗜️ VACUUM 完成: {size_before/1024/1024:.1f}MB → {size_after/1024/1024:.1f}MB (释放 {freed_mb:.1f}MB)")
        except Exception as e:
    ...
    def get_cleanup_stats(self) -> dict:
        ...
        soft_deleted = db.query_local(
            "SELECT COUNT(*) as cnt FROM task_queue WHERE is_deleted = 1"
        )
...
_cleanup_instance: Optional[CleanupService] = None
...
def handle_cleanup_vacuum(payload: dict) -> dict:
    ...
    return {"success": True, "last_vacuum": service._last_vacuum_time.isoformat() if service._last_vacuum_time else None}
